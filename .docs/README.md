...
> BMP 加固已提交为 `aff5632`；请以根目录 `EVIDENCE.md`、实际 `git status` 和最后一次
> 测试命令为准。
...
当前状态（HEAD `aff5632`，24 个提交，249 测试通过）：
...
git log --oneline | cat                    # 应为 24 行，HEAD = aff5632
git status --porcelain | wc -l              # 应为 0
uv run pytest -q                           # 应为 249 passed
