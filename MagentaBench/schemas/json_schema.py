"""JSON Schema generation for the public BMP 0.1 contracts."""

from __future__ import annotations

import json
from pathlib import Path
from typing import Any

from .models import (
    AdapterCapability,
    AdapterCapabilityArtifact,
    BenchmarkArtifactAdapter,
    BenchmarkSpecAdapter,
    CaseOrderArtifact,
    CaseSetActivationReceipt,
    CaseSetArtifact,
    CustomCaseOrderSpec,
    DatasetArtifact,
    DatasetSpec,
    ConfigurationActivationReceipt,
    CheckpointLoadReceipt,
    CheckpointSaveReceipt,
    ClaimReport,
    ConfigurationArtifact,
    ConfigurationSelection,
    ConfigurationSpec,
    CredentialRef,
    EnvironmentReceipt,
    EnvironmentSpec,
    EvidenceBundle,
    EvolutionCandidateRecord,
    EvolutionMethodArtifact,
    EvolutionMethodSpec,
    EvolutionRunEvidence,
    EvolutionSelectionSpec,
    EvolutionSurfacePolicy,
    EvolutionTransitionRecord,
    EvaluatorArtifact,
    EvaluatorSpec,
    ExecutionSpec,
    ExperimentContrast,
    ExperimentRegimeArtifact,
    ExperimentRegimeSpec,
    ExperimentStageSpec,
    FactorArtifact,
    FactorSpec,
    MetricArtifact,
    MetricGroupResult,
    MetricSpec,
    MetricUncertaintyResult,
    MetricUncertaintySpec,
    MetaEvolutionMethodArtifact,
    MetaEvolutionMethodSpec,
    JournalRecord,
    ExternalProtocolAuthorityReceipt,
    IntegrationProbeRecord,
    NetworkObservation,
    ObservationReport,
    ModelActivationReceipt,
    ModelActivationEvidence,
    ModelActivationUsage,
    ProviderBinding,
    RecordIndex,
    ResourceSpec,
    RolloutTrajectory,
    ResolvedBmpManifest,
    ResolvedNetworkPolicy,
    RunReportAdapter,
    ResolvedExecutionSpec,
    ScheduleActivationReceipt,
    StatisticalAnalysisPlan,
    StatisticalAnalysisReceipt,
    SubjectArtifactAdapter,
    SubjectSpecAdapter,
    SystemPromptRecord,
    WorkspaceRecord,
)
from .evolution import (
    EvolutionArchiveEntry,
    EvolutionArchiveLedger,
    EvolutionArchiveSnapshot,
    EvolutionArchiveTransition,
    EvolutionBudgetEvent,
    EvolutionBudgetLedger,
    EvolutionDiagnosisReceipt,
    EvolutionEvaluationRecord,
    EvolutionFailureSample,
    EvolutionFailureSamplingReceipt,
    EvolutionParentSelectionReceipt,
    EvolutionPromotionGateReceipt,
    EvolutionRuntimeReceipt,
    EvolutionSelectionCandidate,
    EvolutionSelectionPolicy,
    EvolutionSealedHoldoutReceipt,
    MetaEvolutionEditPolicy,
    MetaEvolutionEditReceipt,
)
from .registry_lock import RegistryLockCatalog, RegistryLockEntry
from .regime import (
    ContinualAnalysisPlan,
    DerivedMatrixMetricResult,
    ExperimentCellCoordinates,
    ExperimentCellLedger,
    ExperimentCellObservation,
    ExperimentCellPlan,
    ExperimentStageActivationReceipt,
    ExternalMetricAdapterInput,
    ExternalMetricComputationReceipt,
    ExternalMetricGroupResult,
    MetricCellMatrix,
    MetricMatrixCell,
    PlannedMetricCell,
    SealedHoldoutReleaseReceipt,
)
from .research_metrics import (
    CalibrationAnalysisPlan,
    CalibrationMetricsReceipt,
    CalibrationObservation,
    CandidateYieldObservation,
    CandidateYieldPlan,
    EvolutionBudgetCheckpoint,
    EvolutionCurveMetricsReceipt,
    EvolutionCurveObservation,
    EvolutionCurvePlan,
    GeneralizationAnalysisPlan,
    GeneralizationMetricsReceipt,
    MetaEvolutionArmObservation,
    MetaEvolutionPairPlan,
    ParentChildPair,
    ParentDeltaObservation,
    ParentDeltaPlan,
    ProgressCheckpoint,
    ResearchMetricContribution,
    ResearchMetricReceipt,
    SelectorAnalysisPlan,
    SelectorMetricsReceipt,
    SelectorProbabilityObservation,
    TrajectoryAnalysisPlan,
    TrajectoryMetricsReceipt,
    TrajectoryProgressObservation,
)
from .trajectory_ops import (
    CandidateGateCommandReceipt,
    CandidateGateCommandSpec,
    CandidateValidityGateReceipt,
    ContextCompactionReceipt,
    TraceConversionReceipt,
)


def schema_documents() -> dict[str, dict[str, Any]]:
    """Return JSON Schema documents keyed by their canonical file stem."""

    return {
        "benchmark-spec": BenchmarkSpecAdapter.json_schema(),
        "benchmark-artifact": BenchmarkArtifactAdapter.json_schema(),
        "dataset-spec": DatasetSpec.model_json_schema(),
        "dataset-artifact": DatasetArtifact.model_json_schema(),
        "configuration-spec": ConfigurationSpec.model_json_schema(),
        "configuration-selection": ConfigurationSelection.model_json_schema(),
        "configuration-artifact": ConfigurationArtifact.model_json_schema(),
        "configuration-activation-receipt": ConfigurationActivationReceipt.model_json_schema(),
        "adapter-capability": AdapterCapability.model_json_schema(),
        "adapter-capability-artifact": AdapterCapabilityArtifact.model_json_schema(),
        "subject-spec": SubjectSpecAdapter.json_schema(),
        "subject-artifact": SubjectArtifactAdapter.json_schema(),
        "execution-spec": ExecutionSpec.model_json_schema(),
        "experiment-contrast": ExperimentContrast.model_json_schema(),
        "experiment-regime-spec": ExperimentRegimeSpec.model_json_schema(),
        "experiment-regime-artifact": ExperimentRegimeArtifact.model_json_schema(),
        "experiment-stage-spec": ExperimentStageSpec.model_json_schema(),
        "experiment-cell-coordinates": ExperimentCellCoordinates.model_json_schema(),
        "planned-metric-cell": PlannedMetricCell.model_json_schema(),
        "experiment-cell-plan": ExperimentCellPlan.model_json_schema(),
        "experiment-cell-observation": ExperimentCellObservation.model_json_schema(),
        "experiment-cell-ledger": ExperimentCellLedger.model_json_schema(),
        "metric-matrix-cell": MetricMatrixCell.model_json_schema(),
        "metric-cell-matrix": MetricCellMatrix.model_json_schema(),
        "continual-analysis-plan": ContinualAnalysisPlan.model_json_schema(),
        "derived-matrix-metric-result": DerivedMatrixMetricResult.model_json_schema(),
        "sealed-holdout-release-receipt": SealedHoldoutReleaseReceipt.model_json_schema(),
        "experiment-stage-activation-receipt": ExperimentStageActivationReceipt.model_json_schema(),
        "external-metric-adapter-input": ExternalMetricAdapterInput.model_json_schema(),
        "external-metric-group-result": ExternalMetricGroupResult.model_json_schema(),
        "external-metric-computation-receipt": ExternalMetricComputationReceipt.model_json_schema(),
        "calibration-analysis-plan": CalibrationAnalysisPlan.model_json_schema(),
        "calibration-metrics-receipt": CalibrationMetricsReceipt.model_json_schema(),
        "calibration-observation": CalibrationObservation.model_json_schema(),
        "candidate-yield-observation": CandidateYieldObservation.model_json_schema(),
        "candidate-yield-plan": CandidateYieldPlan.model_json_schema(),
        "evolution-budget-checkpoint": EvolutionBudgetCheckpoint.model_json_schema(),
        "evolution-curve-metrics-receipt": EvolutionCurveMetricsReceipt.model_json_schema(),
        "evolution-curve-observation": EvolutionCurveObservation.model_json_schema(),
        "evolution-curve-plan": EvolutionCurvePlan.model_json_schema(),
        "generalization-analysis-plan": GeneralizationAnalysisPlan.model_json_schema(),
        "generalization-metrics-receipt": GeneralizationMetricsReceipt.model_json_schema(),
        "meta-evolution-arm-observation": MetaEvolutionArmObservation.model_json_schema(),
        "meta-evolution-pair-plan": MetaEvolutionPairPlan.model_json_schema(),
        "parent-child-pair": ParentChildPair.model_json_schema(),
        "parent-delta-observation": ParentDeltaObservation.model_json_schema(),
        "parent-delta-plan": ParentDeltaPlan.model_json_schema(),
        "progress-checkpoint": ProgressCheckpoint.model_json_schema(),
        "research-metric-contribution": ResearchMetricContribution.model_json_schema(),
        "research-metric-receipt": ResearchMetricReceipt.model_json_schema(),
        "selector-analysis-plan": SelectorAnalysisPlan.model_json_schema(),
        "selector-metrics-receipt": SelectorMetricsReceipt.model_json_schema(),
        "selector-probability-observation": SelectorProbabilityObservation.model_json_schema(),
        "trajectory-analysis-plan": TrajectoryAnalysisPlan.model_json_schema(),
        "trajectory-metrics-receipt": TrajectoryMetricsReceipt.model_json_schema(),
        "trajectory-progress-observation": TrajectoryProgressObservation.model_json_schema(),
        "factor-spec": FactorSpec.model_json_schema(),
        "factor-artifact": FactorArtifact.model_json_schema(),
        "evaluator-spec": EvaluatorSpec.model_json_schema(),
        "evaluator-artifact": EvaluatorArtifact.model_json_schema(),
        "metric-spec": MetricSpec.model_json_schema(),
        "metric-artifact": MetricArtifact.model_json_schema(),
        "metric-group-result": MetricGroupResult.model_json_schema(),
        "metric-uncertainty-spec": MetricUncertaintySpec.model_json_schema(),
        "metric-uncertainty-result": MetricUncertaintyResult.model_json_schema(),
        "environment-spec": EnvironmentSpec.model_json_schema(),
        "environment-receipt": EnvironmentReceipt.model_json_schema(),
        "resource-spec": ResourceSpec.model_json_schema(),
        "rollout-trajectory": RolloutTrajectory.model_json_schema(),
        "context-compaction-receipt": ContextCompactionReceipt.model_json_schema(),
        "trace-conversion-receipt": TraceConversionReceipt.model_json_schema(),
        "candidate-gate-command-spec": CandidateGateCommandSpec.model_json_schema(),
        "candidate-gate-command-receipt": CandidateGateCommandReceipt.model_json_schema(),
        "candidate-validity-gate-receipt": CandidateValidityGateReceipt.model_json_schema(),
        "credential-ref": CredentialRef.model_json_schema(),
        "case-order-artifact": CaseOrderArtifact.model_json_schema(),
        "case-set-artifact": CaseSetArtifact.model_json_schema(),
        "case-set-activation-receipt": CaseSetActivationReceipt.model_json_schema(),
        "custom-case-order-spec": CustomCaseOrderSpec.model_json_schema(),
        "checkpoint-load-receipt": CheckpointLoadReceipt.model_json_schema(),
        "checkpoint-save-receipt": CheckpointSaveReceipt.model_json_schema(),
        "provider-binding": ProviderBinding.model_json_schema(),
        "model-activation-receipt": ModelActivationReceipt.model_json_schema(),
        "model-activation-evidence": ModelActivationEvidence.model_json_schema(),
        "model-activation-usage": ModelActivationUsage.model_json_schema(),
        "record-index": RecordIndex.model_json_schema(),
        "resolved-execution-spec": ResolvedExecutionSpec.model_json_schema(),
        "evidence-bundle": EvidenceBundle.model_json_schema(),
        "evolution-candidate-record": EvolutionCandidateRecord.model_json_schema(),
        "evolution-method-spec": EvolutionMethodSpec.model_json_schema(),
        "evolution-method-artifact": EvolutionMethodArtifact.model_json_schema(),
        "evolution-selection-spec": EvolutionSelectionSpec.model_json_schema(),
        "evolution-surface-policy": EvolutionSurfacePolicy.model_json_schema(),
        "meta-evolution-method-spec": MetaEvolutionMethodSpec.model_json_schema(),
        "meta-evolution-method-artifact": MetaEvolutionMethodArtifact.model_json_schema(),
        "evolution-transition-record": EvolutionTransitionRecord.model_json_schema(),
        "evolution-run-evidence": EvolutionRunEvidence.model_json_schema(),
        "evolution-selection-policy": EvolutionSelectionPolicy.model_json_schema(),
        "evolution-archive-entry": EvolutionArchiveEntry.model_json_schema(),
        "evolution-archive-snapshot": EvolutionArchiveSnapshot.model_json_schema(),
        "evolution-archive-transition": EvolutionArchiveTransition.model_json_schema(),
        "evolution-archive-ledger": EvolutionArchiveLedger.model_json_schema(),
        "evolution-selection-candidate": EvolutionSelectionCandidate.model_json_schema(),
        "evolution-parent-selection-receipt": EvolutionParentSelectionReceipt.model_json_schema(),
        "evolution-promotion-gate-receipt": EvolutionPromotionGateReceipt.model_json_schema(),
        "registry-lock-entry": RegistryLockEntry.model_json_schema(),
        "registry-lock-catalog": RegistryLockCatalog.model_json_schema(),
        "evolution-budget-event": EvolutionBudgetEvent.model_json_schema(),
        "evolution-budget-ledger": EvolutionBudgetLedger.model_json_schema(),
        "evolution-failure-sample": EvolutionFailureSample.model_json_schema(),
        "evolution-failure-sampling-receipt": EvolutionFailureSamplingReceipt.model_json_schema(),
        "evolution-diagnosis-receipt": EvolutionDiagnosisReceipt.model_json_schema(),
        "meta-evolution-edit-policy": MetaEvolutionEditPolicy.model_json_schema(),
        "meta-evolution-edit-receipt": MetaEvolutionEditReceipt.model_json_schema(),
        "evolution-evaluation-record": EvolutionEvaluationRecord.model_json_schema(),
        "evolution-runtime-receipt": EvolutionRuntimeReceipt.model_json_schema(),
        "evolution-sealed-holdout-receipt": EvolutionSealedHoldoutReceipt.model_json_schema(),
        "network-observation": NetworkObservation.model_json_schema(),
        "resolved-network-policy": ResolvedNetworkPolicy.model_json_schema(),
        "journal-record": JournalRecord.model_json_schema(),
        "integration-probe-record": IntegrationProbeRecord.model_json_schema(),
        "external-protocol-authority-receipt": ExternalProtocolAuthorityReceipt.model_json_schema(),
        "system-prompt-record": SystemPromptRecord.model_json_schema(),
        "workspace-record": WorkspaceRecord.model_json_schema(),
        "claim-report": ClaimReport.model_json_schema(mode="serialization"),
        "observation-report": ObservationReport.model_json_schema(),
        "run-report": RunReportAdapter.json_schema(mode="serialization"),
        "schedule-activation-receipt": ScheduleActivationReceipt.model_json_schema(),
        "statistical-analysis-plan": StatisticalAnalysisPlan.model_json_schema(),
        "statistical-analysis-receipt": StatisticalAnalysisReceipt.model_json_schema(),
        "resolved-bmp-manifest": ResolvedBmpManifest.model_json_schema(),
    }


def write_json_schemas(output_dir: str | Path) -> tuple[Path, ...]:
    """Generate deterministic, human-readable ``*.schema.json`` files."""

    destination = Path(output_dir)
    destination.mkdir(parents=True, exist_ok=True)
    written: list[Path] = []
    for name, schema in sorted(schema_documents().items()):
        path = destination / f"{name}.schema.json"
        path.write_text(
            json.dumps(schema, sort_keys=True, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        written.append(path)
    return tuple(written)


__all__ = ["schema_documents", "write_json_schemas"]
